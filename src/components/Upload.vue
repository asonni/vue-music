<template>
  <div class="bg-white rounded border border-gray-200 relative flex flex-col">
    ...
    <div class="p-6">
      ...
      <hr class="my-6" />
      <!-- Progress Bars -->
      <div v-for="uploadItem in uploads" :key="uploadItem.name" class="mb-4">
        <!-- File Name -->
        <div class="font-bold text-sm" :class="uploadItem.textClass">
          <i :class="uploadItem.icon" /> {{ uploadItem.name }}
        </div>
        <div class="flex h-4 overflow-hidden bg-gray-200 rounded">
          <!-- Inner Progress Bar -->
          <div
            class="transition-all progress-ba"
            :class="uploadItem.variant"
            :style="{ width: uploadItem.currentProgress + '%' }"
          />
        </div>
      </div>
    </div>
